...
public class caminoDestruccion
{
    ...
    {
        files.printTexto.textoEfectoEscritura("Te apresuras hacia la pequeña puerta en la esquina de la sala, consigues entrar en ella pero no puedes cerrarla del todo. En consecuencia, el agua está entrando por la ranura y debes aguantar la respiración.\r\n" + //
                                                        "\r\n" + //
    ...
    {
        files.printTexto.textoEfectoEscritura("Te aproximas al nido donde descansan las aves y las analizas. Nunca habías visto una criatura similar, pero antes de plantearte pasar por la otra puerta comienzan a correr hacia ti. Son crías y tienen hambre, su madre no parece estar por los alrededores.\r\n" + //
                                                                "\r\n" + //
        ...
        siguesVivo = files.combate.simularCombate(2*dificultad, "aveCria", player);
        if(!siguesVivo)
        {
            return siguesVivo;
        }
        files.printTexto.textoEfectoEscritura("Sigues avanzando y reconoces un sonido, es una criatura piando, está furiosa. Antes de poder localizar de dónde viene el sonido, un ave de un tamaño similar a un buitre se aproxima hacia tu posición. Es la madre de las pequeñas crías hambrientas.");                                
        siguesVivo = files.combate.simularCombate(1*dificultad, "aveMadre", player);   
        if (!siguesVivo)
        {
            return siguesVivo;
        }
        files.printTexto.textoEfectoEscritura("Tras la dura lucha con el ave continúas tu camino.\r\n" + //
                                                                "\r\n" + //
    ...
    {
        files.printTexto.textoEfectoEscritura("Te aterran los insectos, es algo en lo que no puedes evitar pensar cada vez que vas a explorar. Esta vez realmente estás en un apuro por lo que comienzas a pisotear y aplastar con el mango de tu mandoble todo ser que se acerque a ti. ");
        files.combate.simularCombate(5*dificultad, "bicho", player);
        files.printTexto.textoEfectoEscritura("Tras unos cuantos, o podría decirse bastantes escalofríos, consigues acabar con la mayoría de insectos a tu alrededor.\r\n" + //
                                                    "\r\n" + //
    ...
    public static boolean espejos(jugador player, int dificultad, boolean siguesVivo)
    {
        ...
        siguesVivo = files.combate.simularCombate(4*dificultad, "lagarto", player);
        if (!siguesVivo)
        {
            return siguesVivo;
        }
        files.printTexto.textoEfectoEscritura("Coges tu mandoble y comienzas a atacar de lado a lado pero pierdes la orientación debido a los espejos, que dificultan la batalla. Procuras saltar por encima de las amenazas ya que es un movimiento que no pueden realizar fácilmente. Una vez acabas con ellos, inspeccionas el agujero y encuentras la salida.");
        return siguesVivo;
    }
    ...
    public static boolean rocas(jugador player, int dificultad, boolean siguesVivo)
    {
        ...
        siguesVivo = files.Ahorcado.ahorcado("moneda", player);
        if (!siguesVivo)
        {
            return siguesVivo;
        }
    ...
    {
        files.printTexto.textoEfectoEscritura("Entras en la sala aparentemente inofensiva. El techo tiene una estructura extraña, está dividido en diferentes paneles con barras verticales, como si de una celda se tratase. La puerta por la que has entrado se cierra, pero la de salida está abierta.\r\n" + //
                                                        "\r\n" + //
        ...
        siguesVivo = files.combate.simularCombate(1*dificultad, "mambaNegra", player);
        if (!siguesVivo)
        {
            return siguesVivo;
        }
        files.printTexto.textoEfectoEscritura(" Tras numerosos esquivos y amagos, consigues cortar a la mamba en dos. Dejan de caer las placas y, cansado, te aproximas a la puerta, ya abierta.");
        return siguesVivo;
    }
    ...
    public static void Destruccion(jugador player, int dificultad)
    {
        ...
        files.printTexto.textoEfectoEscritura("Eliges el camino del centro, tras una larga sesión de evitar arbustos, serpientes, ramas y más apareces en lo que se asimila a un antiguo escenario de batalla en ruinas.\r\n" + //
                        "\r\n" + //
        ...
        if (eleccion == 1)
        {
            siguesVivo = chaac(player, dificultad, siguesVivo);
            if (!siguesVivo)
            {
                return ;
            }
            ...
            if (eleccion == 1)
            {
                siguesVivo = puertaSuelo(player, dificultad, siguesVivo);
                if (!siguesVivo)
                {
                    return ;
                }
                ...
                if (eleccion == 1)
                {
                    siguesVivo = nido(player, dificultad, siguesVivo);
                    if (!siguesVivo)
                    {
                        return ;
                    }                             
                ...
                {
                    monedaAcertijo(player, dificultad);
                    FinalDestruccion(player, dificultad, false);
                }
            }
            //puerta techo
            else
            {
                ...
                if (eleccion == 1)
                {
                    files.printTexto.textoEfectoEscritura("Esta vez te encuentras en una inmensa sala repleta de vegetación por las paredes y el techo. La vegetación es variada y colorida pero puedes distinguir miles de insectos entre la flora. Intentas avanzar con cuidado en caso de que supongan una amenaza pero no tiene efecto, en un abrir y cerrar de ojos, incontables insectos de distintos tipos corren, vuelan y reptan hacia ti.\r\n" + //
                                                            "\r\n" + //
                    ...
                    if (eleccion == 1)
                    {
                        files.printTexto.textoEfectoEscritura("Corres hacia el pasadizo huyendo de tantos insectos como puedes, el pasadizo es oscuro pero tropiezas con un objeto pesado. Enciendes el mechero que llevas en el bolsillo y examinas el objeto. Se trata de un cofre un tanto extraño, tiene unas formas curvas que crean un relieve parecido a unos tentáculos y tiene un color entre verdoso y morado, tampoco presenta ningún reflejo o brillo.\r\n" + //
                                                                    "\r\n" + //
                    ...
                    {
                        estatuaAcertijo(player,dificultad);
                ...
                {
                    siguesVivo = salaOscua(player,dificultad, siguesVivo);
                    if (!siguesVivo)
                    {
                        return ;
                    }
        ...
        {
            files.printTexto.textoEfectoEscritura("Te acercas a la puerta de Kauil y aparecen dos leones que debes derrotar. Una vez acabas con ellos, la puerta de Kauil se abre. Hay una rampa por la que debes deslizarte.\r\n" + //
                                    "\r\n" + //
            ...
            if (eleccion == 1)
            {
                files.printTexto.textoEfectoEscritura("Entras por el agujero que desprende una luz radiante, una vez dentro te encuentras en una sala con un enorme agujero por donde entra la luz del sol. Es extraño, jurarías que antes de bajar por la rampa el cielo no estaba tan claro.\r\n" + //
                                            "\r\n" + //
                ...
                if (eleccion == 1)
                {
                    siguesVivo = espejos(player, dificultad, siguesVivo);
                    if (!siguesVivo)
                    {
                        return ;
                    }
                ...
                {
                    siguesVivo = rocas(player, dificultad, siguesVivo);
                    if (!siguesVivo)
                    {
                        return ;
                    }
            ...
            {
                files.printTexto.textoEfectoEscritura("Te adentras en el agujero corriente, te mata la curiosidad sobre qué es lo que brilla en el interior del otro, pero sientes que este es más seguro. Caes por una rampa y acabas en una sala de temperatura muy fría. Debes estar mucho más bajo tierra de lo que estabas antes.\r\n" + //
                                            "\r\n" + //
                ...
                {
                    siguesVivo = izquierdaDestr(player, dificultad, siguesVivo);
                    if (!siguesVivo)
                    {
                        return ;
                    }
